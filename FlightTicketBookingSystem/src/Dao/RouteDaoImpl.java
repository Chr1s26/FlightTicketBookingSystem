...
public class RouteDaoImpl implements RouteDao {
	...
	public List<Route> getAllRoutes() {
		...
		try(Connection connection = connectionFactory.createConnection()) {
			...
			while(resultSet.next()) {
				...
				int flight = resultSet.getInt("flight_id");
				Flight flight2 = flightDao.getFlightByFlightId(flight);
				routes.add(new Route(deptairport, arrivalairport, distance, flight2));
			}
	...
	public Route getRouteById(int id) {
		...
		try(Connection connection = connectionFactory.createConnection()) {
			...
			if(resultSet.next()) {
				...
				int flight = resultSet.getInt("flight_id");
				Flight flight2 = flightDao.getFlightByFlightId(flight);
				route = new Route(deptairport, arrivalairport, distance,flight2);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void addRoute(Route route) {
		...
		try(Connection connection = connectionFactory.createConnection()) {
			...
			preparedStatement.setInt(4, route.getFlight().getFlightid());
